module test.codec.http2.hpack.HpackEncoderTest;
...
class HpackEncoderTest {
    ...
    void testNeverIndexSetCookie() {
        ...
        BufferUtils.flipToFlush(buffer, 0);
        tracef("position=%d, limit=%d, remaining=%d", buffer.position(), buffer.limit(), buffer.remaining());
        tracef("xxxx=>%(%02X %)", buffer.array()[0.. buffer.limit()+16]);
    ...
    //     assertThat((buffer.get(buffer0.remaining()) & 0xFF) >> 6, (1));

    //     // check first field is static index name and literal body
    //     assertThat((buffer.get(buffer1.remaining()) & 0xFF) >> 4, (0));

    //     // check first field is static index name and dynamic index body
    //     assertThat((buffer.get(buffer2.remaining()) & 0xFF) >> 6, (1));

    //     // Only first and third fields are put in the table
    //     HpackContext context = encoder.getHpackContext();
    //     assertThat(context.size(), (2));
    //     assertThat(context.get(HpackContext.STATIC_SIZE + 1).getHttpField().getName(), ("host"));
    //     assertThat(context.get(HpackContext.STATIC_SIZE + 2).getHttpField().getName(), ("user-agent"));
    //     assertThat(context.getDynamicTableSize(), (
