...
class TestMatcher {
    ...
    void testHuntRouter() {
        ...
        Router router0 = routerManager.register().get("/hello/get");
        Router router1 = routerManager.register().get("/hello/:testParam0");
        Router router3 = routerManager.register().post("/book/update/:id");
        Router router4 = routerManager.register().post("/book/update/{id<\\d+>}");
        
        NavigableSet!(RouterMatchResult) result = routerManager.findRouter("POST", "/book/update/a3", null, null);
        assert(result.size() == 1);
        assert("a3" == result.first().getParameters().get("id"));
        
        //
        result = routerManager.findRouter("POST", "/book/update/43", null, null);
        assert(result.size() == 2);
        assert(result.first().getParameters().get("id") == "43");
        assert(result.last().getParameters().get("id") == "43");
