...
class HttpFieldsTest {
	...
	// 	HttpGenerator.putTo(header, buffer);
	// 	BufferUtils.flipToFlush(buffer, 0);
	// 	string o = BufferUtils.toString(buffer).toLower();

	// 	Assert.assertThat(o, Matchers.containsString(
	// 			(HttpHeader.CONNECTION ~ ": " ~ HttpHeaderValue.KEEP_ALIVE).toLower()));
	// 	Assert.assertThat(o, Matchers.containsString(
	// 			(HttpHeader.TRANSFER_ENCODING ~ ": " ~ HttpHeaderValue.CHUNKED).toLower()));
	// 	Assert.assertThat(o, Matchers.containsString(
	// 			(HttpHeader.CONTENT_ENCODING ~ ": " ~ HttpHeaderValue.GZIP).toLower()));
	// }
