...
class HttpServerHandler : AbstractHttpHandler {
    ...
    override void sessionOpened(Session session) {
        ...
        if (config.isSecureConnectionEnabled()) {
            ...
            SecureSession secureSession = factory.create(session, false,
                    (SecureSession sslSession) {
                        ...
                        case "http/1.1":
                            ...
                            break;
                        case "h2":
                            httpConnection = new Http2ServerConnection(config,
                                session, sslSession, listener);
                            break;
                        default:
            ...
            if (!config.getProtocol().empty) {
                ...
                if (httpVersion == HttpVersion.Null) {
                    string msg = "the protocol " ~ config.getProtocol() ~ " is not support.";
                    warning(msg);
