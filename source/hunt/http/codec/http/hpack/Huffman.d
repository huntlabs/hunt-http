...
class Huffman
{
    ...
    __gshared  int[][] LCCODES; // = new int[CODES.length][];
    
    // Huffman decode tree stored in a flattened char array for good 
    // locality of reference.
    __gshared  ubyte[] tree;
    __gshared  ubyte[] rowsym;
    __gshared  byte[] rowbits;
    ...
    static string decode(ByteBuffer buffer,int length)
    {        
        ...
        while (bits > 0) 
        {
            ...
            // tracef("node=%d, c=%d", node, c);
            node = tree[node*256+c];
        // tracef("rowbits[%d]=>%d", node, rowbits[node]);
    ...
    static int octetsNeededLC(string s)
    {
        return octetsNeeded(LCCODES,s);
    }

    static void encodeLC(ByteBuffer buffer, string s)
    {
